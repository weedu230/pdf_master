...
export default function RepairPdfPage() {
  ...
  const handleRepair = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/repair`, formData, {
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.setAttribute('download', 'repaired.pdf');
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
      window.URL.revokeObjectURL(url);
    } catch (err) {
      setError(err.response?.data?.detail || 'Error repairing PDF');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>Repair Damaged PDF - PDF Master</title>
        <meta name="description" content="Repair corrupted or damaged PDF files. Fix common PDF issues easily." />
        <meta name="keywords" content="repair pdf, fix corrupted pdf, damaged pdf repair online" />
      </Helmet>

      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-12 md:py-20">
        <div className="max-w-4xl mx-auto px-4 md:px-6">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3 md:mb-4 animate-fade-in-up">Repair PDF</h1>
          <p className="text-base md:text-lg text-gray-600 mb-8 md:mb-12 animate-fade-in-up-delay-1">
            Repair damaged or corrupted PDF files. Fix formatting and structure issues.
          </p>
