...
export default function PowerpointToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/powerpoint-to-pdf`, formData, {
        responseType: 'blob',
      });
      ...
      setError(err.response?.data?.detail || 'Error converting PowerPoint to PDF');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>PowerPoint to PDF Converter - PDF Master</title>
        <meta name="description" content="Convert PowerPoint presentations to PDF. Fast and reliable PPTX to PDF conversion." />
        <meta name="keywords" content="powerpoint to pdf, pptx to pdf converter, convert powerpoint to pdf online" />
      </Helmet>

      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-12 md:py-20">
        <div className="max-w-4xl mx-auto px-4 md:px-6">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3 md:mb-4 animate-fade-in-up">PowerPoint to PDF</h1>
          <p className="text-base md:text-lg text-gray-600 mb-8 md:mb-12 animate-fade-in-up-delay-1">
            Convert your PowerPoint presentations to PDF format. Perfect for sharing and archiving.
          </p>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 md:p-8 animate-fade-in-up-delay-2">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 md:p-8 text-center">
              <p className="text-sm md:text-base text-gray-600 mb-4">Supports: .ppt, .pptx</p>
              <FileUpload onFileSelect={setFile} acceptedTypes=".ppt,.pptx" />
            </div>
